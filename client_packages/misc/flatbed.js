...
function createBed(veh) {
  return new Promise((resolve, reject) => {
    ...
    veh.bed.state = 0
    setTimeout(() => {
      if(veh.bed && veh.handle)
      {
        veh.bed.attachTo(veh.handle, veh.getBoneIndexByName('chassis_dummy'), 0,-3,-0.467, 14,0,180, true, false, false, false,0,true)
      }
      resolve()
    }, 155)
  });
    ...
    keypressCheck = setInterval(() => {
      ...
      if (mp.game.controls.isControlJustPressed(0, hookKey)) {
        if (isDrivingFlatbed() && !flatbed.bed.moving && flatbed.bed.state == 1
          ...
          if (targetVeh && targetVeh.getClass() != 19 && targetVeh.getClass() != 10 && targetVeh.getClass() != 14 && targetVeh.getClass() != 15 && targetVeh.getClass() != 16 && targetVeh.getClass() != 17 && targetVeh.getClass() != 20 && targetVeh.getClass() != 21) {
  ...
  entityStreamOut(e) {
    if (e.type == 'vehicle' && e.model == mp.game.joaat('flatbed')) {
      if (e.bed) {
        ...
        if (e.bed.rope != null)
          mp.game.rope.detachRopeFromEntity(e.bed.rope, e);
...
function attachToBed(flatbed, targetVeh) {
  ...
  if (targetVeh === false) {
    ...
    targetVeh.attachTo(flatbed.bed.handle, 0, 0, 0, height-0.315, rotX, 0, rotZ, true, false, false, false, 0, true)
